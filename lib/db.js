...
export async function listOrgStructure() {
  ...
  const sections = await sql`SELECT * FROM sections ORDER BY name ASC`;
  const units = await sql`SELECT * FROM units ORDER BY name ASC`;
  const deptCounts = await sql`
    SELECT department, COUNT(*)::int AS count FROM users WHERE role = 'employee' GROUP BY department
  `;
  const unitCounts = await sql`
    SELECT department, unit, COUNT(*)::int AS count FROM users WHERE role = 'employee' AND unit IS NOT NULL GROUP BY department, unit
  `;
...
// ---- HR onboarding ---------------------------------------------------------------
export async function onboardEmployee({ name, email, password, department, position, role, unit }) {
  await ensureReady();
  const sql = getSql();
  const [row] = await sql`
    INSERT INTO users (name, email, password_hash, role, department, position, join_date, unit)
    VALUES (${name}, ${email}, ${bcrypt.hashSync(password, 8)}, ${role || "employee"}, ${department || "General"}, ${position || "Employee"}, ${new Date().toISOString().slice(0, 10)}, ${unit ?? null})
    RETURNING *
  `;
  return toClient(row);
}
