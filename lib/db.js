...
async function initSchemaAndSeed() {
  ...
  await sql`
    CREATE TABLE IF NOT EXISTS company_settings (
      ...
      check_in_deadline TEXT NOT NULL DEFAULT '10:00',
  ...
  `;
  await sql`
    INSERT INTO company_settings (id, check_in_deadline, working_hours, weekend_days)
    VALUES (1, '10:00', 8, '5,6')
...
export async function clockIn(userId) {
  ...
  const [deadlineH, deadlineM] = settings.checkInDeadline.split(":").map(Number);
...
export async function getSettings() {
  ...
  return {
    checkInDeadline: row?.check_in_deadline || "10:00",
    workingHours: row ? Number(row.working_hours) : 8,
    weekendDays: (row?.weekend_days || "5,6").split(",").map(Number),
  };
}
export async function updateSettings({ checkInDeadline, workingHours, weekendDays }) {
  await ensureReady();
  const sql = getSql();
...
export async function addHoliday({ name, startDate, endDate, type }) {
  await ensureReady();
  const sql = getSql();
  const [row] = await sql`
    INSERT INTO holidays (name, start_date, end_date, type)
    VALUES (${name}, ${startDate}, ${endDate || startDate}, ${type || "holiday"})
    RETURNING *
  `;
  return row;
}
...
// ---- HR onboarding ---------------------------------------------------------------
export async function onboardEmployee({ name, email, password, department, position, role }) {
  await ensureReady();
  const sql = getSql();
  const [row] = await sql`
    INSERT INTO users (name, email, password_hash, role, department, position, join_date)
    VALUES (${name}, ${email}, ${bcrypt.hashSync(password, 8)}, ${role || "employee"}, ${department || "General"}, ${position || "Employee"}, ${new Date().toISOString().slice(0, 10)})
    RETURNING *
  `;
  return toClient(row);
}
