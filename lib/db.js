...
export async function updateLeaveStatus(id, status, reviewNote) {
  ...
  const sql = getSql();
  const [row] = await sql`
    UPDATE leave_requests SET status = ${status}, review_note = COALESCE(${reviewNote}, review_note)
...
export async function updateServiceStatus(id, status, reviewNote) {
  ...
  const sql = getSql();
  const [row] = await sql`
    UPDATE service_requests SET status = ${status}, review_note = COALESCE(${reviewNote}, review_note)
    WHERE id = ${Number(id)} RETURNING *
  `;
  return row;
}
