...
// ---- HR onboarding ---------------------------------------------------------------
export async function onboardEmployee({ name, email, password, department, position, role, unit, nationalId, phone, emergencyContactName, emergencyContactPhone }) {
  await ensureReady();
  const sql = getSql();
  const [row] = await sql`
    INSERT INTO users (name, email, password_hash, role, department, position, join_date, unit, national_id, phone, emergency_contact_name, emergency_contact_phone)
    VALUES (${name}, ${email}, ${bcrypt.hashSync(password, 8)}, ${role || "employee"}, ${department || "General"}, ${position || "Employee"}, ${new Date().toISOString().slice(0, 10)}, ${unit ?? null}, ${nationalId ?? null}, ${phone ?? null}, ${emergencyContactName ?? null}, ${emergencyContactPhone ?? null})
    RETURNING *
  `;
  return toClient(row);
}
