...
async function initSchemaAndSeed() {
  ...
  await sql`ALTER TABLE users ADD COLUMN IF NOT EXISTS basic_salary NUMERIC`;
  await sql`ALTER TABLE users ADD COLUMN IF NOT EXISTS default_allowances NUMERIC`;
  await sql`ALTER TABLE users ADD COLUMN IF NOT EXISTS bank_account_number TEXT`;
  await sql`ALTER TABLE users ADD COLUMN IF NOT EXISTS status TEXT NOT NULL DEFAULT 'active'`;
  await sql`ALTER TABLE users ADD COLUMN IF NOT EXISTS retired_date DATE`;
  await sql`
  ...
  `;
  await sql`ALTER TABLE attendance ADD COLUMN IF NOT EXISTS break_in TEXT`;
  await sql`ALTER TABLE attendance ADD COLUMN IF NOT EXISTS break_out TEXT`;
  await sql`
  ...
  `;
  await sql`ALTER TABLE service_requests ADD COLUMN IF NOT EXISTS effective_date DATE`;
  await sql`
...
export async function listEmployees() {
  ...
  const rows = await sql`
    SELECT id, name, email, role, department, position, join_date, unit, status,
           retired_date AS "retiredDate", (photo IS NOT NULL) AS "hasPhoto"
...
export async function listAttendanceForUser(userId) {
  ...
  const sql = getSql();
  return sql`
    SELECT *, check_in AS "checkIn", check_out AS "checkOut", break_in AS "breakIn", break_out AS "breakOut"
...
export async function listAllAttendance() {
  ...
  `;
  return rows.map((r) => ({
    id: r.id, userId: r.user_id, date: r.date, checkIn: r.check_in, checkOut: r.check_out,
    breakIn: r.break_in, breakOut: r.break_out, status: r.status,
...
export async function getTodayAttendance(userId) {
  ...
  if (!row) return null;
  return { id: row.id, userId: row.user_id, date: row.date, checkIn: row.check_in, checkOut: row.check_out, breakIn: row.break_in, breakOut: row.break_out, status: row.status };
}
...
export async function breakIn(userId) {
  await ensureReady();
  const sql = getSql();
  const existing = await getTodayAttendance(userId);
  if (!existing || !existing.checkIn || existing.checkOut) return null;
  const now = new Date();
  const time = `${String(now.getHours()).padStart(2, "0")}:${String(now.getMinutes()).padStart(2, "0")}`;
  const [row] = await sql`UPDATE attendance SET break_in = ${time} WHERE id = ${existing.id} RETURNING *`;
  return row;
}
export async function breakOut(userId) {
  await ensureReady();
  const sql = getSql();
  const existing = await getTodayAttendance(userId);
  if (!existing || !existing.breakIn || existing.breakOut) return null;
  const now = new Date();
  const time = `${String(now.getHours()).padStart(2, "0")}:${String(now.getMinutes()).padStart(2, "0")}`;
  const [row] = await sql`UPDATE attendance SET break_out = ${time} WHERE id = ${existing.id} RETURNING *`;
  return row;
}
...
// Returns employees eligible to have salary generated for a given month/year,
// split into "with overtime" / "without overtime" buckets. Excludes anyone who
// already has a slip for that period, and anyone with a still-pending time
// change request in that period (their attendance isn't regularized yet).
// Overtime is derived from attendance vs. that day's effective work hours
// (holiday/Ramadan/custom-day adjusted, or the company default) - computed in
// one batched pass instead of per-day/per-employee queries.
export async function listSalaryGenerationCandidates(month, year) {
  await ensureReady();
  const sql = getSql();
  ...
  const [employees, existingSlips, pendingChanges, attendance, holidays, settings] = await Promise.all([
    sql`
      SELECT id, name, department, position, basic_salary AS "basicSalary",
             default_allowances AS "defaultAllowances", bank_account_number AS "bankAccountNumber"
      FROM users WHERE role = 'employee' ORDER BY name ASC
    `,
    sql`SELECT DISTINCT user_id FROM salary_slips WHERE month = ${m} AND year = ${y}`,
    sql`SELECT DISTINCT user_id FROM time_change_requests WHERE status = 'pending' AND date BETWEEN ${startDate} AND ${endDate}`,
    sql`SELECT user_id, date, check_in, check_out FROM attendance WHERE date BETWEEN ${startDate} AND ${endDate} AND check_in IS NOT NULL AND check_out IS NOT NULL`,
    sql`
      ...
      FROM holidays WHERE adjusted_start_time IS NOT NULL AND start_date <= ${endDate} AND end_date >= ${startDate}
    `,
    getSettings(),
  ]);

  const alreadyGenerated = new Set(existingSlips.map((r) => r.user_id));
  const pendingReview = new Set(pendingChanges.map((r) => r.user_id));

  const toMinutes = (t) => {
    const [h, mm] = t.split(":").map(Number);
    return h * 60 + (mm || 0);
  };
  const effectiveHoursFor = (dateStr) => {
    const override = holidays.find((h) => dateStr >= h.startDate && dateStr <= h.endDate);
    return {
      start: override?.adjustedStartTime || settings.workStartTime,
      end: override?.adjustedEndTime || settings.workEndTime,
    };
  };

  const otMinutesByUser = new Map();
  for (const a of attendance) {
    const { start, end } = effectiveHoursFor(a.date);
    const standardMinutes = Math.max(0, toMinutes(end) - toMinutes(start));
    const workedMinutes = Math.max(0, toMinutes(a.check_out) - toMinutes(a.check_in));
    const otMinutes = Math.max(0, workedMinutes - standardMinutes);
    otMinutesByUser.set(a.user_id, (otMinutesByUser.get(a.user_id) || 0) + otMinutes);
  }
  ...
  for (const e of employees) {
    if (alreadyGenerated.has(e.id)) continue;
    if (pendingReview.has(e.id)) {
      pendingReviewCount++;
      continue;
    }
    const otMinutes = otMinutesByUser.get(e.id) || 0;
    const entry = { ...e, otHours: Math.round((otMinutes / 60) * 10) / 10 };
    (otMinutes > 0 ? withOT : withoutOT).push(entry);
  }

  return { withOT, withoutOT, alreadyGeneratedCount: alreadyGenerated.size, pendingReviewCount };
}
...
export async function listServicesForUser(userId) {
  ...
  const sql = getSql();
  const rows = await sql`
    SELECT *, applied_on AS "appliedOn", review_note AS "reviewNote", effective_date AS "effectiveDate"
...
export async function listAllServices() {
  ...
  return rows.map((r) => ({
    ...
    effectiveDate: r.effective_date, status: r.status, appliedOn: r.applied_on, reviewNote: r.review_note,
    user: { id: r.u_id, name: r.u_name, email: r.u_email, role: r.u_role, department: r.u_department, position: r.u_position, joinDate: r.u_join_date },
  }));
}
export async function addServiceRequest(data) {
  ...
  const sql = getSql();
  const [row] = await sql`
    INSERT INTO service_requests (user_id, type, amount, details, effective_date, status, applied_on, review_note)
    VALUES (${data.userId}, ${data.type}, ${data.amount ?? null}, ${data.details}, ${data.effectiveDate ?? null}, 'pending', ${new Date().toISOString().slice(0, 10)}, '')
    RETURNING *
  `;
  return row;
}
...
export async function listOrgStructure() {
  ...
  const [departments, sections, units, deptCounts, unitCounts] = await Promise.all([
    ...
    sql`SELECT department, COUNT(*)::int AS count FROM users WHERE role = 'employee' AND status != 'retired' GROUP BY department`,
    sql`SELECT department, unit, COUNT(*)::int AS count FROM users WHERE role = 'employee' AND status != 'retired' AND unit IS NOT NULL GROUP BY department, unit`,
  ]);
...
// ---- HR onboarding ---------------------------------------------------------------
export async function onboardEmployee({ name, email, password, department, position, role, unit, nationalId, phone, emergencyContactName, emergencyContactPhone, reportsTo, basicSalary, defaultAllowances, bankAccountNumber }) {
  await ensureReady();
  const sql = getSql();
  const [row] = await sql`
    INSERT INTO users (name, email, password_hash, role, department, position, join_date, unit, national_id, phone, emergency_contact_name, emergency_contact_phone, reports_to, basic_salary, default_allowances, bank_account_number)
    VALUES (${name}, ${email}, ${bcrypt.hashSync(password, 8)}, ${role || "employee"}, ${department || "General"}, ${position || "Employee"}, ${new Date().toISOString().slice(0, 10)}, ${unit ?? null}, ${nationalId ?? null}, ${phone ?? null}, ${emergencyContactName ?? null}, ${emergencyContactPhone ?? null}, ${reportsTo ? Number(reportsTo) : null}, ${basicSalary ?? null}, ${defaultAllowances ?? null}, ${bankAccountNumber ?? null})
    RETURNING *
  `;
  return toClient(row);
}
...
export async function updateEmployeePayroll(userId, { basicSalary, defaultAllowances, bankAccountNumber }) {
  await ensureReady();
  const sql = getSql();
  const [row] = await sql`
    UPDATE users
    SET basic_salary = ${basicSalary ?? null},
        default_allowances = ${defaultAllowances ?? null},
...
export async function retireEmployee(userId, retiredDate) {
  await ensureReady();
  const sql = getSql();
  const [row] = await sql`
    UPDATE users SET status = 'retired', retired_date = ${retiredDate || new Date().toISOString().slice(0, 10)}
    WHERE id = ${Number(userId)}
    RETURNING *
  `;
  return toClient(row);
...
export async function reactivateEmployee(userId) {
  ...
  const sql = getSql();
  const [row] = await sql`
    UPDATE users SET status = 'active', retired_date = NULL
    WHERE id = ${Number(userId)}
    RETURNING *
  `;
  return toClient(row);
